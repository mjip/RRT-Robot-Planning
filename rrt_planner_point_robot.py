...
try:
    ...
    print "usage: rrt_planner_point_robot.py <image>"
...
def rrt_search(G, tx, ty):
    ...
    while dist_from_goal > SMALLSTEP:
        iteration += 1
        dist_from_goal = pointPointDistance(vertices[closestPointToPoint(G, (tx, ty))], (tx, ty))
        rand_point = genPoint()
        near_point = closestPointToPoint(G,rand_point)
        near_point_v = vertices[near_point]
        new_point = map(lambda pair: pair[0] + SMALLSTEP * pair[1], zip(near_point_v, lineFromPoints(near_point_v, rand_point)))
        ...
        if new_point[0] >= XMAX or new_point[0] < 1 or new_point[1] >= YMAX or new_point[1] < 1:
            ...
            for obstacle in obstacles:
                if lineHitsRect(near_point_v, new_point, obstacle) or inRect(new_point, obstacle, 0.5):
                    in_boundary = False
                    break
        if in_boundary:
